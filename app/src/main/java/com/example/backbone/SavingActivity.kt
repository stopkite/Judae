...
class SavingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val categoryList = ArrayList<PopUpListData>()

        // 카테고리에 들어갈 목록들 삽입(임의로 넣은 데이터) : 기능 연결할 때 이 부분 다뤄줘야함...
        categoryList.add(PopUpListData("우정"))
        categoryList.add(PopUpListData("사랑"))
        categoryList.add(PopUpListData("진로"))
        categoryList.add(PopUpListData("가족"))
        categoryList.add(PopUpListData("감정"))

        // adapter 초기화
        popUpAdapter = PopUpAdapter(this,categoryList)

        // 리스트뷰에 방금 생성한 adapter를 붙여서 화면에 연결해준다.
        cateList.adapter = popUpAdapter
