...
import com.juksooon.backbone.databinding.ActivityMyQuestionBinding
...
class MyQuestionActivity : AppCompatActivity() {
    ...
    override fun onResume(){
        ...
        // 질문 리스트 데이터 넣기
        for (i in 0..(Array.size - 1)) {
            qList.add(MyQListData(resources.getDrawable(R.drawable.ic_q_list_question), "${Array[i].Content}", "${Array[i].Date}","${Array[i].WritingID}"))
        }
        ...
        // 만든 어댑터 리사이클러뷰에 연결
        binding.myQQlist.adapter = qAdapter
    }
